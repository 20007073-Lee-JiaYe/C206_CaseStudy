...
//  private ArrayList<Order> orderLists;
  ...
  public void viewLunchBoxTest() {
    ...
    C206_CaseStudy.viewLunchBoxOrder(orderList);
    testOutput = String.format("%-30d %10s %10s %10s %10s\n", 123, "1-Aug-2021", "Burger", "Orange Juice",
        "Apple Slice");
    testOutput += String.format("%-30d %10s %10s %10s %10s\n", 124, "2-Aug-2021", "dumpling soup",
        "apple juice", "Honey Dew");
...
//    String deleteorderid = "123";
    assertSame("Test if the order id exists in the user account list", 2, orderList.size());

    // Test if the order list size is 1 when 1 order is deleted
    C206_CaseStudy.viewLunchBoxOrder(orderList);
    orderList.remove(order1);
    System.out.println("Deleted successfully");
    assertEquals("Test if the user account list size is 2 to 1", 1, orderList.size());
